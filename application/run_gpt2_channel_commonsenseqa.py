...
def main(args):
    ...
    aggregate_decoder_attn = torch.zeros(
        0,
    )
    context_types = []
    all_prediction_type = []
    ...
    for _dir in [gold_dir, random_dir]:
        ...
        input_attributions = torch.cat(
            (
                input_attributions,
                torch.load(pjoin(_dir, "input_attributions.pt"))[:, :max_len, :max_len],
            ),
            dim=0,
        )
        head_attributions = torch.cat(
            (head_attributions, torch.load(pjoin(_dir, "head_attributions.pt"))), dim=0
        )

        context_types += [
            "gold" if _dir == gold_dir else "random" for _ in range(len(hidden_states))
        ]

        aggregate_decoder_attn = torch.load(pjoin(_dir, "aggregate_attentions.pt"))[
            :, :, :max_len, :max_len
        ]
    ...
    # Place-holders
    torch.save(
        head_attributions.mean(0), os.path.join(args.output_dir, "decoder_head_importance.pt")
    )
