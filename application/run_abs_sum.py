...
def main(args):
    ...
    for i, example in enumerate(tqdm(dataset)):
        ...
        _id = example["id"]
        batch = tokenizer([article], truncation=True, padding="longest", return_tensors="pt").to(
            device
        )
        ...
        #     encoder_attention = (
        #         torch.stack(encoder_attention).squeeze(1).to(dtype=torch.float16)
        #     )  # n_layer x n_heads x input_len x input_len

        #     # output_len x n_layers x beam_size x n_heads x 1 x output_len
        #     decoder_attention = output.decoder_attentions
        #     decoder_attention = [
        #         torch.stack(attn)[:, 0].squeeze(2).to(dtype=torch.float16)
        #         for attn in decoder_attention
        #     ]  # output_len x n_layers x n_heads x n_tokens_prior
        ...
        #         [torch.stack(attn)[:, 0].squeeze(2) for attn in cross_attention]
    ...
    ### temp: load hidden_states pt files shared from google drive
    encoder_hiddens = torch.load(pjoin(args.output_dir, "encoder_hidden_states.pt"))
