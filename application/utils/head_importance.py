...
def compute_taylor_importance_score(
    model, attn, layer_index, n_heads, head_size, importance_scores, input_embeddings=None
):
    pruned_heads = set()
    ...
    for head_idx in leftover_heads:
        heads, index = find_pruneable_heads_and_indices(
            [head_idx], n_heads, head_size, pruned_heads
        )
        ...
        query_W_grad = (
            attn.q_proj.weight.grad.index_select(0, index)
            * attn.q_proj.weight.index_select(0, index)
        ) ** 2

        key_b_grad = (attn.k_proj.bias.grad[index] * attn.k_proj.bias[index]) ** 2
        key_W_grad = (
            attn.k_proj.weight.grad.index_select(0, index)
            * attn.k_proj.weight.index_select(0, index)
        ) ** 2

        value_b_grad = (attn.v_proj.bias.grad[index] * attn.v_proj.bias[index]) ** 2
        value_W_grad = (
            attn.v_proj.weight.grad.index_select(0, index)
            * attn.v_proj.weight.index_select(0, index)
        ) ** 2

        output_W_grad = (
            attn.out_proj.weight.grad.index_select(1, index)
            * attn.out_proj.weight.index_select(1, index)
        ) ** 2
        abs_grad_magnitude = (
            query_b_grad.sum()
            + query_W_grad.sum()
...
def compute_ig_importance_score(
    model, attn, layer_index, n_heads, head_size, importance_scores, input_embeddings
):
...
def get_head_importance_pegasus(model, method="taylor", input_ids=None):
    ...
    if method not in methods:
        raise NotImplementedError(
            f"Attribution method '{method}' for attention is not yet supported"
        )
    ...
    if method == "taylor":
        for i in range(n_layers_encoder):
            ...
            importance_scores_encoder = attribution_fn(
                model,
    ...
    for i in range(n_layers_decoder):
        ...
        importance_scores_decoder = attribution_fn(
            model,
            self_attention_decoder,
            i,
            n_heads_decoder,
            head_size_decoder,
            importance_scores_decoder,
            input_embeddings,
        )
        importance_scores_cross = attribution_fn(
            model,
            cross_attention,
            i,
            n_heads_encoder,
            head_size_encoder,
            importance_scores_cross,
            input_embeddings,
        )

    return {
        "encoder": importance_scores_encoder,
...
def get_taylor_importance(model):
    ...
    for i in range(n_layers):
        ...
        for head_idx in leftover_heads:
            heads, index = find_pruneable_heads_and_indices(
                [head_idx], num_attention_heads, head_size, pruned_heads
            )
            index = index.to(model.device)

            query_b_grad = (
                attention.self.query.bias.grad[index] * attention.self.query.bias[index]
            ) ** 2
            query_W_grad = (
                attention.self.query.weight.grad.index_select(0, index)
                * attention.self.query.weight.index_select(0, index)
            ) ** 2

            key_b_grad = (attention.self.key.bias.grad[index] * attention.self.key.bias[index]) ** 2
            key_W_grad = (
                attention.self.key.weight.grad.index_select(0, index)
                * attention.self.key.weight.index_select(0, index)
            ) ** 2

            value_b_grad = (
                attention.self.value.bias.grad[index] * attention.self.value.bias[index]
            ) ** 2
            value_W_grad = (
                attention.self.value.weight.grad.index_select(0, index)
                * attention.self.value.weight.index_select(0, index)
            ) ** 2

            output_W_grad = (
                attention.output.dense.weight.grad.index_select(1, index)
                * attention.output.dense.weight.index_select(1, index)
            ) ** 2
            abs_grad_magnitude = (
                query_b_grad.sum()
                + query_W_grad.sum()
