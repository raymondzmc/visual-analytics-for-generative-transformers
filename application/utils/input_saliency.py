...
def bert_layer_lrp(layer, relevance):
    if relevance.shape != layer.output.activation.shape:
        ...
        relevance_holder[
            :, 0
        ] = relevance  # Since only the CLS token is used for classfication (Could also be repeated)
    ...
    relevance_residual_ff = torch.autograd.grad(
        layer.output.activation,
        layer.output.input[1],
        grad_outputs=relevance,
        retain_graph=True,
        allow_unused=True,
    )[0]
    ...
    relevance = torch.autograd.grad(
        layer.output.activation,
        layer.output.dense.activation,
        grad_outputs=relevance,
        retain_graph=True,
        allow_unused=True,
    )[0]
    relevance = lrp_linear(
        layer.output.dense.weight, layer.output.dense.bias, layer.output.dense.input[0], relevance
    )
    relevance = lrp_linear(
        layer.intermediate.dense.weight,
        layer.intermediate.dense.bias,
        layer.intermediate.dense.input[0],
        relevance,
    )
    relevance += relevance_residual_ff

    # Relevance for the self-attention output transformation
    relevance_residual_attn = torch.autograd.grad(
        layer.attention.output.activation,
        layer.attention.output.input[1],
        grad_outputs=relevance,
        retain_graph=True,
        allow_unused=True,
    )[0]
    ...
    relevance = torch.autograd.grad(
        layer.attention.output.activation,
        layer.attention.output.dense.activation,
        grad_outputs=relevance,
        retain_graph=True,
        allow_unused=True,
    )[0]
    relevance = lrp_linear(
        ...
        layer.attention.output.dense.input[0],
        relevance,
    )
    ...
    relevance_key = torch.autograd.grad(
        self_attention.activation[0],
        key.activation,
        grad_outputs=relevance,
        retain_graph=True,
        allow_unused=True,
    )[0]
    relevance_query = torch.autograd.grad(
        self_attention.activation[0],
        query.activation,
        grad_outputs=relevance,
        retain_graph=True,
    ...
    relevance_value = torch.autograd.grad(
        self_attention.activation[0],
        value.activation,
        grad_outputs=relevance,
        retain_graph=True,
...
def bert_lrp(model, out_relevance, grad=None):
    ...
    for name, module in module_list:
        if name == "classifier":
            ...
            relevance = lrp_linear(
                module.dense.weight, module.dense.bias, module.dense.input[0], relevance
            )
...
def bert_ig(input_tokens):
    ...
    input_ids, ref_input_ids, sep_id = construct_input_ref_pair(
        input_tokens, ref_token_id, sep_token_id, cls_token_id
    )
